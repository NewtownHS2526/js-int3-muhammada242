...
console.log(weeklyAllowance / 3);

// 2. How many games can Jordan buy with their cut?
console.log(Math.floor((weeklyAllowance / 3) / game));
...
console.log(Math.floor(((weeklyAllowance / 3) * 2 * 4) / shoes));

// 5. How many more games can they buy if they put their money together compared to if they had bought games on their own?
console.log(Math.floor(((weeklyAllowance / 3) * 2) / game) - Math.floor((weeklyAllowance / 3) / game) * 2);
...
console.log(sibling3.substring(15, 27));
...
const buyMeals = (budget) => {
  ...
  const leftover = Math.round((budget - (numMeals * mealCost)) * 100) / 100;
  return `You can buy ${numMeals} meals and have $${leftover} left over.`;
};
...
const missingLeg = (side1, side2) => {
  ...
  const missingLegLength = Math.sqrt(hypotenuse * hypotenuse - leg * leg);
  console.log(missingLegLength);
};
